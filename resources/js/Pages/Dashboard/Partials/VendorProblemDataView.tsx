...
export default function ({ data }: { data: PaginateResponse<VendorProblemResource> }) {
    ...
    const [progressResponse, setProgressResponse] =
        useState<PaginateResponse<VendorProblemResource>>();
                ...
                <Table>
                    <TableHeader>
                        <TableRow>
                            <TableHead>
                                {t(
                                    'pages.dashboard.partials.vendor_problem_data_view.headers.vendor_name',
                                )}
                            </TableHead>
                            <TableHead>
                                {t(
                                    'pages.dashboard.partials.vendor_problem_data_view.headers.total_sent',
                                )}
                            </TableHead>
                            <TableHead>
                                {t(
                                    'pages.dashboard.partials.vendor_problem_data_view.headers.total_problem',
                                )}
                            </TableHead>
                        </TableRow>
                    </TableHeader>
                    <TableBody>
                        {progressResponse.data.map((data, index) => (
                            <TableRow key={`${index}_${data.vendor_name}`}>
                                ...
                                <TableCell>
                                    {data.total_problem} ({data.problem_percent}%)
                                </TableCell>
                            </TableRow>
                        ))}
                    </TableBody>
